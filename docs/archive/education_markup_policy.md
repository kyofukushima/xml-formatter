...
> **【アーカイブ】この文書は旧仕様の記録であり、現在のツールの動作を表していません。**（2026年9月アーカイブ）
>
> 本文書は、科目構成を Item / Subitem1〜3 の固定4段階に変換する専用スクリプト（`scripts/education_script/convert_list_to_item.py`）の仕様として作成されました。当該スクリプトは現在存在せず、科目名の括弧書き Item を扱う処理は `scripts/convert_subject_item.py` として本体パイプラインに組み込まれています。本体パイプラインは Subitem10 までの階層、設定ファイル駆動のラベル判定（丸数字・ローマ数字・アルファベット等）、表・図の配置と検証に対応しており、本文書「5.2 制限事項」に記載の制限（Subitem4 以降、表・図の混在、番号体系の限定）は現在のツールには当てはまりません。
>
> 現行の仕様は [README.md](../../README.md)、[処理ロジック.md](../../処理ロジック.md)、[GUIDE.md](../../GUIDE.md) を参照してください。
