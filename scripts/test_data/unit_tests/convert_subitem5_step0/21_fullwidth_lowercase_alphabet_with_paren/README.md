...
- Subitem4要素内のList要素で、1つ目のColumnに「ａ）」「ｂ）」などの全角小文字アルファベット + ）のラベルがある
- このListがSubitem5要素に変換される
- 1つ目のColumnの内容（例：「ａ）」）がSubitem5Titleに配置される
- 2つ目のColumnの内容がSubitem5Sentenceに配置される
...
- Subitem4要素内のColumnなしListは、前のSubitem5要素（ColumnありListから変換されたSubitem5）にList要素として追加されることを確認
