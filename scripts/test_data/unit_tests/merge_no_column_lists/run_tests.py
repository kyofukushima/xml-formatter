...
"""
ColumnなしList統合（--merge-no-column-lists）の単体テスト実行スクリプト

convert_item_step0.py（script.txt で指定した場合はそのスクリプト）を対象に、
連続する ColumnなしList の Column（LineBreak）統合の動作を検証する。

各テストケースディレクトリの構成:
  input.xml    - 入力XML
  expected.xml - 期待される出力XML
  args.txt     - (オプション) スクリプトへの追加引数（例: --merge-no-column-lists）
                 ファイルがない場合はフラグなし（従来動作）で実行される
  script.txt   - (オプション) 実行する変換スクリプト名（例: convert_subitem1_step0.py）
                 ファイルがない場合は convert_item_step0.py を実行する
"""
...
def run_test(test_dir):
    ...
    # 実行するスクリプト（デフォルト: convert_item_step0.py。script.txt で上書き可）
    script_file = test_dir / "script.txt"
    script_name = "convert_item_step0.py"
    if script_file.exists():
        script_name = script_file.read_text(encoding='utf-8').strip() or script_name
...
def main():
    ...
    print("ColumnなしList統合（--merge-no-column-lists）単体テスト実行")
    print("=" * 50)
