...
実データ（告示の数式規定）を模したパターンで、画像List（数式のQuoteStruct/Fig）の後に数式の変数説明のテキストListが連続する場合の並列分割ロジックを、Subitem10レベルでテストします。
...
- 画像ListがSubitem10要素に変換される（Subitem10Titleは空、Subitem10SentenceにQuoteStruct入りSentenceを保持）
- 後続の変数説明のテキストListは、同一Subitem10Sentence内のSentence連続（Num=2, 3, …）として統合されず、それぞれ別々のSubitem10要素に変換される（並列分割）
