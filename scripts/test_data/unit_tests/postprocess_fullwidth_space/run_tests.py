...
"""
postprocess_fullwidth_space.py の単体テスト実行スクリプト

各テストケースディレクトリの構成:
  input.xml    - 入力XML
  expected.xml - 期待される出力XML
  args.txt     - (オプション) スクリプトへの追加引数（例: --mode remove --include-list）
"""
...
def run_test(test_dir):
    ...
    # postprocess_fullwidth_space.py を実行
    script_path = Path(__file__).parent.parent.parent.parent / "postprocess_fullwidth_space.py"
...
def main():
    ...
    print("postprocess_fullwidth_space.py 単体テスト実行")
    print("=" * 50)
