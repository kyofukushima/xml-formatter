...
"""
表・図の後のList保護（--preserve-lists-after-struct）の単体テスト実行スクリプト

親要素直下に TableStruct/FigStruct/StyleStruct が置かれた後の List を
変換せず保持する動作を検証する。対象スクリプトはケースごとに script.txt で指定する
（Item内のListを扱う convert_subitem1_step0.py が中心。Paragraph直下が対象外であることは
convert_item_step0.py、Subitem階層でも効くことは convert_subitem2_step0.py で確認）。

各テストケースディレクトリの構成:
  input.xml    - 入力XML
  expected.xml - 期待される出力XML
  args.txt     - (オプション) スクリプトへの追加引数（例: --preserve-lists-after-struct）
                 ファイルがない場合はフラグなし（従来動作）で実行される
  script.txt   - (オプション) 実行する変換スクリプト名（デフォルト: convert_subitem1_step0.py）
"""
...
def run_test(test_dir):
    ...
    # 対象スクリプト（script.txt で指定。デフォルトは convert_subitem1_step0.py）
    script_name = "convert_subitem1_step0.py"
    if script_file.exists():
        script_name = script_file.read_text(encoding='utf-8').strip()
    print(f"対象スクリプト: {script_name}")
    script_path = Path(__file__).parent.parent.parent.parent / script_name
...
def main():
    ...
    print("表・図の後のList保護（--preserve-lists-after-struct）単体テスト実行")
    print("=" * 50)
