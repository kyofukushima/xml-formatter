...
"""
normalize_double_paren.py の単体テスト実行スクリプト

各テストケースディレクトリの構成:
  input.xml    - 入力XML
  expected.xml - 期待される出力XML
  args.txt     - (オプション) スクリプトへの追加引数（現状は未使用）
"""
...
def run_test(test_dir):
    ...
    # normalize_double_paren.py を実行
    script_path = Path(__file__).parent.parent.parent.parent / "normalize_double_paren.py"
...
def main():
    ...
    print("normalize_double_paren.py 単体テスト実行")
    print("=" * 50)
