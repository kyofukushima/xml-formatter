...
実データ（告示の数式規定）を模したパターンで、画像List（数式のQuoteStruct/Fig）の後に数式の変数説明のテキストListが連続する場合の並列分割ロジックを、Subitem1レベルでテストします。
...
- 画像ListがSubitem1要素に変換される（Subitem1Titleは空、Subitem1SentenceにQuoteStruct入りSentenceを保持）
- 後続の変数説明のテキストListは、同一Subitem1Sentence内のSentence連続（Num=2, 3, …）として統合されず、それぞれ別々のSubitem1要素に変換される（並列分割）
