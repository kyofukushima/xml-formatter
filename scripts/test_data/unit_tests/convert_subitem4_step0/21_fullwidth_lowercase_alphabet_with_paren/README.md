...
- Subitem3要素内のList要素で、1つ目のColumnに「ａ）」「ｂ）」などの全角小文字アルファベット + ）のラベルがある
- このListがSubitem4要素に変換される
- 1つ目のColumnの内容（例：「ａ）」）がSubitem4Titleに配置される
- 2つ目のColumnの内容がSubitem4Sentenceに配置される
...
- Subitem3要素内のColumnなしListは、前のSubitem4要素（ColumnありListから変換されたSubitem4）にList要素として追加されることを確認
