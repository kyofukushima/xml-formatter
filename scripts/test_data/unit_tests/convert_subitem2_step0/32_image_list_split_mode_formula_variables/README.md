...
実データ（告示の数式規定）を模したパターンで、画像List（数式のQuoteStruct/Fig）の後に数式の変数説明のテキストListが連続する場合の並列分割ロジックを、Subitem2レベルでテストします。
...
- 画像ListがSubitem2要素に変換される（Subitem2Titleは空、Subitem2SentenceにQuoteStruct入りSentenceを保持）
- 後続の変数説明のテキストListは、同一Subitem2Sentence内のSentence連続（Num=2, 3, …）として統合されず、それぞれ別々のSubitem2要素に変換される（並列分割）
