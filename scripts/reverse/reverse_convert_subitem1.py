...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
使用例:
  python3 reverse_convert_subitem1.py input.xml
  python3 reverse_convert_subitem1.py input.xml output.xml
        '''
    )

    parser.add_argument('input_file', help='入力XMLファイル')
    parser.add_argument('output_file', nargs='?', help='出力XMLファイル（デフォルト: _reverse_subitem1.xml）')
    ...
    output_path = Path(args.output_file) if args.output_file else input_path.parent / f"{input_path.stem}_reverse_subitem1.xml"
