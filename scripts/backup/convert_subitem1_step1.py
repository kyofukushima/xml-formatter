...
def find_split_point_for_column_list(subitem1_elem: etree.Element) -> Optional[int]:
    """ColumnありListを探す関数（最初のColumnありListは分割しない）"""
    children = list(subitem1_elem)

    # Subitem1Sentenceのインデックスを探す
    subitem1_sentence_index = -1
    for i, child in enumerate(children):
        if child.tag == 'Subitem1Sentence':
            subitem1_sentence_index = i
            break

    if subitem1_sentence_index == -1:
        return None

    # Subitem1Sentenceの後の最初のColumnありListを探す（2番目以降のもの）
    for i in range(subitem1_sentence_index + 1, len(children)):
        child = children[i]
        if child.tag != 'List':
            continue
