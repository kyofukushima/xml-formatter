...
def find_first_split_point_for_subitem5(subitem5_elem: etree.Element, config: ConversionConfig) -> Optional[Tuple[int, str]]:
    ...
    # Subitem5Sentenceを探す
    subitem5_sentence_index = -1
    for i, child in enumerate(children):
        if child.tag == 'Subitem5Sentence':
            subitem5_sentence_index = i
            break

    if subitem5_sentence_index == -1:
        return None

    # Subitem5Sentenceの次の弟要素を確認（すべてのListをチェック）
    for i in range(subitem5_sentence_index + 1, len(children)):
        child = children[i]
        if child.tag == 'List':
            # ColumnありListは分割対象
