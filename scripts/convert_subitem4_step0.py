...
"""
Subitem4要素変換ロジック実装スクリプト（共通化版）

xml_converter.pyを使用してSubitem3要素内のList要素をSubitem4要素に変換する。
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Subitem4要素変換ロジック実装スクリプト（共通化版）',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
使用例:
  python3 convert_subitem4_step0.py input.xml
  python3 convert_subitem4_step0.py input.xml output.xml
        '''
    )

    parser.add_argument('input_file', help='入力XMLファイル')
    parser.add_argument('output_file', nargs='?', help='出力XMLファイル（デフォルト: _subitem4_step0.xml）')
    ...
    output_path = Path(args.output_file) if args.output_file else input_path.parent / f"{input_path.stem}_subitem4_step0.xml"

    # Subitem4変換の設定（学年パターンもサポート）
