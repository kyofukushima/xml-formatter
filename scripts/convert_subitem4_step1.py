...
def find_first_split_point_for_subitem4(subitem4_elem: etree.Element, config: ConversionConfig) -> Optional[Tuple[int, str]]:
    ...
    # Subitem4Sentenceを探す
    subitem4_sentence_index = -1
    for i, child in enumerate(children):
        if child.tag == 'Subitem4Sentence':
            subitem4_sentence_index = i
            break

    if subitem4_sentence_index == -1:
        return None

    # Subitem4Sentenceの次の弟要素を確認（すべてのListをチェック）
    for i in range(subitem4_sentence_index + 1, len(children)):
        child = children[i]
        if child.tag == 'List':
            # ColumnありListは分割対象
