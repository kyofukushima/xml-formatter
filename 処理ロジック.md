...
- **ColumnなしList統合オプション（`--merge-no-column-lists`。WebアプリではデフォルトON、CLIでは指定時のみ）**  
  告示データ整備方針①に合わせ、連続するColumnなしList（段落）を個別の要素に分割せず、1つの要素の `*Sentence` 内に `Column`（`LineBreak="true"`）として統合します。Paragraph直下では空TitleのItemを1つ作ってColumnを並べ、Titleあり要素（「（１）　見出し」等）の本文直後では見出しを `Column Num="1"` に包んで段落を `Column Num="2"` 以降に畳み込みます（空TitleのSubitemは作りません）。ラベル付きListや表が現れた時点で統合を終了します。
  ```xml
