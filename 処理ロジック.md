...
- **ColumnなしList統合オプション（`--merge-no-column-lists`、デフォルトOFF）**  
...
- **ColumnなしListの分割モード（設定ファイル、デフォルトON）**  
  上記の統合オプションがOFFのとき、連続するColumnなしList（段落）をどう扱うかは `label_config.json` の `conversion_behaviors` で決まります。`no_column_text_split_mode` がONなら段落ごとに別々のItem/Subitem（Titleは空）に並列分割し、OFFなら直前の要素にListのまま取り込みます。`image_list_split_mode` は画像のみのList（QuoteStruct）由来の要素の後に続くテキスト段落を並列分割するかを決めます。いずれもWebアプリの「ラベル設定管理」ページで切り替えられます。変換前後の例は `変換モード切り替え.md` を参照してください。
